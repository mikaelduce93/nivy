    ...
    <div className="space-y-8 pt-6 pb-8">
      ...
      <header className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: "Niveau", value: stats.level, icon: Star, color: "text-teal" },
            { label: "Coins", value: stats.coins.toLocaleString(), icon: Coins, color: "text-coral" },
            { label: "Rang", value: stats.rank ? `#${stats.rank}` : "-", icon: Trophy, color: "text-teal" },
            { label: "Amis", value: stats.friendsCount, icon: Users, color: "text-pink" },
          ].map((stat) => (
        ...
        {/* Tabs */}
        <HubTabs tabs={PROFILE_TABS} defaultTab="profile" ariaLabel="Sections du profil" />
      </header>
  ...
  return (
    <div className="space-y-6">
      ...
      <StickerCard className="p-6">
        <h3 className="eyebrow tracking-[0.16em] mb-4">Bio</h3>
        <p className="text-ink-2">
          {profile?.bio || "Pas encore de bio. Appuie sur Modifier pour en ajouter une !"}
        </p>
      </StickerCard>
      ...
      <StickerCard className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="eyebrow tracking-[0.16em]">Trophées</h3>
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-mute">Progression</span>
      ...
      <StickerCard variant="panel" className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-ink">Confidentialité</p>
            <p className="text-xs text-mute">Ton profil n&apos;est visible que par tes amis</p>
          </div>
        </div>
      </StickerCard>
    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Meilleur streak", value: stats.bestStreak, icon: Flame, color: "text-coral", suffix: " j" },
          { label: "Badges", value: stats.badges, icon: Award, color: "text-gold", suffix: "" },
          { label: "Events", value: stats.eventsAttended, icon: Calendar, color: "text-lime", suffix: "" },
          { label: "Missions", value: stats.missionsCompleted, icon: Target, color: "text-teal", suffix: "" },
        ].map((stat) => (
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        setActivities([
          { id: '1', type: 'streak', text: `Streak de ${stats.currentStreak} jours active !`, time: 'Maintenant' }
        ])
      } finally {
        setLoading(false)
      }
  ...
  if (activities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <h3 className="font-display text-xl font-extrabold text-ink mt-4 mb-2">Pas encore d&apos;activité</h3>
        <p className="text-mute max-w-sm">
          Termine des quêtes et explore l&apos;app pour voir ton activité ici !
        </p>
      </div>
    )
  }
