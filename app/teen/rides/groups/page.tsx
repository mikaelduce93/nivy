/**
 * /teen/rides/groups — Group rides hub (V6, issue #235).
 *
 * Lists the teen's ride group_actions split into:
 * - « Mes sorties organisées » (organizer)
 * - « Invitations reçues » (invitee)
 *
 * Reads group_actions + group_action_invites server-side (RLS lets the teen
 * read their own as organizer/participant), matching the direct-Supabase
 * pattern of app/teen/rides/page.tsx. Charte paper : StickerCard + pills mono.
 */
...
export default async function TeenRideGroupsPage() {
    ...
    <div className="container mx-auto space-y-8 pt-6">
        ...
        <div className="flex min-w-0 items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <h1 className="font-display text-4xl font-extrabold tracking-tight">
              Vos <em className="font-semibold italic text-pink">sorties</em>
            </h1>
            <p className="mt-1 text-sm text-mute">
              Organise un trajet à plusieurs, invite tes amis et partagez la note.
            </p>
          </div>
        </div>
        <Link href="/teen/rides/groups/new" className="shrink-0">
      ...
      <section className="space-y-3">
        ...
        {organized.length === 0 ? (
          <NivEmpty
            mood="calm"
            title="Aucune sortie organisée"
            description="Crée une sortie pour réserver un trajet avec tes amis et débloquer une remise selon le nombre de participants."
            action={
              <Button asChild variant="pink" className="min-h-11">
                <Link href="/teen/rides/groups/new">Nouvelle sortie</Link>
              </Button>
            }
          />
        ) : (
      ...
      <section className="space-y-3">
        ...
        {invited.length === 0 ? (
          <NivEmpty
            ...
            description="Quand un ami t'invite à une sortie, elle apparaît ici pour que tu acceptes ou refuses."
          />
        ) : (
          <div className="space-y-3">
            {invited.map((a) => (
