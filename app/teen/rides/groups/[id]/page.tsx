/**
 * /teen/rides/groups/[id] — Group ride detail (V6, issue #235).
 *
 * Server component: loads the group_action, its roster (group_action_invites
 * joined to teen names), and any unlocked rewards. Renders:
 * - roster (accepted / pending) with status pills
 * - GroupRideActions (client): discount preview, accept/decline (invitee),
 *   and « Finaliser la course » (organizer).
 */
...
import { GroupRideActions } from "./actions-client"
...
export default async function GroupRideDetailPage({ params }: PageProps) {
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0">
          <p className="eyebrow tracking-[0.16em]">
            Sortie · {GROUP_STATUS[action.status] ?? action.status}
          </p>
