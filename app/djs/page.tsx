  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-background via-background to-muted/20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-16">
              <Card>
                <CardContent className="p-6 text-center">
                  <div className="text-3xl font-bold text-lime">{djs?.length || 0}</div>
              ...
              <Card>
                <CardContent className="p-6 text-center">
                  <div className="text-3xl font-bold text-pink">4.8</div>
                  <div className="text-sm text-muted-foreground">Note Moyenne</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6 text-center">
                  <div className="text-3xl font-bold text-lime">10K+</div>
                  ...
                  <Card key={dj.id} className="group hover:shadow-xl transition-all duration-300 overflow-hidden">
                    <div className="relative h-64 overflow-hidden">
                      ...
                      <div className="absolute bottom-4 left-4 right-4">
                        <h3 className="text-2xl font-bold text-ink mb-1">{dj.stage_name}</h3>
                        <p className="text-ink/80">{dj.name}</p>
                      </div>
                      {dj.rating && (
                        <div className="absolute top-4 right-4 bg-ink/70  rounded-full px-3 py-1 flex items-center gap-1">
                          <Star className="h-4 w-4 fill-gold text-gold" />
                          <span className="text-ink font-semibold">{dj.rating}</span>
                        </div>
                      )}
                    </div>
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t">
                        <div>
                          <div className="text-2xl font-bold text-lime">{dj.hourly_rate} DH</div>
                          <div className="text-xs text-muted-foreground">par heure</div>
                        </div>
                        <Link href={`/djs/${dj.id}`}>
                          <Button className="bg-gradient-to-r from-lime to-coral-600 hover:from-lime hover:to-coral-700">
                            Voir Profil
                          </Button>
                        </Link>
                      </div>
        ...
        {/* CTA Section */}
        <section className="py-20 px-4 bg-gradient-to-r from-lime via-coral-600 to-pink">
          <div className="max-w-4xl mx-auto text-center text-ink space-y-6">
            <h2 className="text-4xl font-bold">Besoin d'un DJ pour votre événement?</h2>
            <p className="text-xl text-ink/90">Réservez maintenant et offrez à vos ados une soirée inoubliable</p>
