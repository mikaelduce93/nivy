...
export default function DevenirPartenairePage() {
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 py-24 relative">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-mute mb-8 leading-relaxed">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Button asChild size="lg" className="bg-gradient-to-r from-teal to-pink hover:opacity-90 text-lg px-8">
                <Link href="/devenir-partenaire/inscription">
                  Commencer maintenant
                  <ArrowRight className="ml-2 w-5 h-5" />
                </Link>
              </Button>
              <Button asChild variant="outline" size="lg" className="border-ink text-ink hover:bg-card text-lg px-8">
      ...
      {/* Stats Section */}
      <div className="border-y border-ink bg-card">
        <div className="container mx-auto px-6 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {STATS.map((stat, i) => (
              <div key={i} className="text-center">
                <stat.Icon className="w-8 h-8 text-teal mx-auto mb-3" />
                <div className="text-3xl md:text-4xl font-black text-ink mb-1">{stat.value}</div>
                <div className="text-mute text-sm">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {PARTNER_TYPES.map((partnerType) => (
            <Link
              ...
              className="group"
            >
              <div className={`relative bg-card rounded-2xl overflow-hidden border border-ink hover:border-ink transition-all h-full ${partnerType.bgColor} hover:scale-[1.02]`}>
                <div className="p-8">
                  ...
                  <h3 className="text-2xl font-bold text-ink mb-2">{partnerType.title}</h3>
                  <p className="text-mute mb-6">{partnerType.description}</p>
                ...
                <div className={`p-4 bg-gradient-to-r ${partnerType.gradient} text-ink font-semibold text-center`}>
      ...
      {/* CTA Section */}
      <div className="bg-gradient-to-r from-teal/10 via-pink/10 to-pink/10 border-y border-ink">
