...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await admin.rpc("request_ride", {
      ...
      p_event_id: body.eventId ?? undefined,
      p_provider: body.provider ?? "nivy_partner",
      p_payment_method: body.paymentMethod ?? "coins",
      p_pickup_lat: body.pickupLat ?? undefined,
      p_pickup_lng: body.pickupLng ?? undefined,
      p_dropoff_lat: body.dropoffLat ?? undefined,
      p_dropoff_lng: body.dropoffLng ?? undefined,
      p_estimated_dh: body.estimatedDh ?? undefined,
