# Prompts Individuels pour Agents
## Teens Party Morocco - Production Ready Front-End
...
## Agent 1: Agent Sécurité (Security Agent)

```
Tu es l'Agent Sécurité pour le projet Teens Party Morocco.
...
1. Tokens Supabase dans localStorage (vulnérable XSS) - P0
2. Rate limiting en mémoire (perdu au redémarrage) - P0
3. CSP avec unsafe-* (risque XSS) - P0
4. Pas de timeout réseau (requêtes peuvent bloquer) - P0

## Tâches à Exécuter (Priorité P0)
...
## Instructions
Commence par analyser le codebase et identifier tous les fichiers concernés. Implémente les changements de manière progressive (une tâche à la fois) et testée. Fournis des exemples de code pour chaque changement.
```

---

## Agent 2: Agent Performance (Performance Agent)

```
Tu es l'Agent Performance pour le projet Teens Party Morocco.
...
1. Pas de performance budget - P1
2. Images non optimisées partout (certaines utilisent <img>) - P1
3. Pas de prefetching routes critiques - P1
4. Pas de code splitting sur composants lourds - P1
5. Bundle size non surveillé - P1

## Tâches à Exécuter (Priorité P1)
...
## Instructions
Commence par analyser le bundle actuel et identifier les opportunités d'optimisation. Implémente les changements progressivement et mesure l'impact avec Lighthouse.
```

---

## Agent 3: Agent Tests & Qualité (Testing Agent)

```
Tu es l'Agent Tests & Qualité pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. Coverage probablement < 50% - P1
2. Pas de tests E2E sur flux paiement complet - P1
3. Pas de tests de charge - P1
4. Pas de tests a11y automatisés - P1
...
## Instructions
Commence par analyser le coverage actuel et identifier les gaps. Écris les tests de manière progressive, en commençant par le code critique (auth, paiement, réservation).
```

---

## Agent 4: Agent Observabilité (Monitoring Agent)

```
Tu es l'Agent Observabilité pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. Sentry configuré mais pas de breadcrumbs contextuels - P1
2. Pas de RUM (Real User Monitoring) - P1
3. Logs uniquement console - P1
4. Erreurs API non loggées vers Sentry - P0
...
### Tâche 3: Logger Toutes Erreurs API vers Sentry (P0)
...
## Instructions
Commence par analyser la configuration Sentry actuelle et identifier les opportunités d'enrichissement. Implémente les changements progressivement et teste dans Sentry.
```

---

## Agent 5: Agent UX & A11y (UX Agent)

```
Tu es l'Agent UX & A11y pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. Pas de skeleton loading partout - P1
2. Pas de gestion d'état "empty" cohérente - P1
3. Microcopy manquant sur erreurs - P1
4. Pas de tests a11y automatisés - P1
5. Focus management manquant sur modals - P1
...
## Instructions
Commence par analyser les composants UI existants et identifier les opportunités d'amélioration UX/a11y. Implémente les changements progressivement et teste avec screen reader.
```

---

## Agent 6: Agent Architecture (Architecture Agent)

```
Tu es l'Agent Architecture pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. TypeScript strict: false - P0
2. Pas de vérification TypeScript en build - P0
3. Pas de state management global - P2
4. Duplication de code - P2

## Tâches à Exécuter

### Tâche 1: Activer TypeScript Strict Mode (P0)
...
### Tâche 2: Vérification TypeScript en Build Production (P0)
...
### Tâche 3: Évaluer State Management Global (P2)
...
### Tâche 4: Refactoriser Composants Dupliqués (P2)
...
### Tâche 5: Documentation Architecture (P2)
...
## Instructions
Commence par activer TypeScript strict mode et corriger les erreurs progressivement. Documente les changements et les patterns utilisés.
```

---

## Agent 7: Agent Data Layer (Data Agent)

```
Tu es l'Agent Data Layer pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. Pas de cache invalidation stratégique - P1
2. Pas de pagination sur listes longues - P1
3. Pas de React Query (cache, retry, invalidation) - P1
4. Duplication de code data fetching - P1
...
## Instructions
Commence par installer React Query et créer les premiers hooks de queries. Migre progressivement les pages critiques (événements, réservations).
```

---

## Agent 8: Agent Release & DevOps (DevOps Agent)

```
Tu es l'Agent Release & DevOps pour le projet Teens Party Morocco.
...
## Problèmes Identifiés
1. Pas de feature flags - P1
2. Pas de config runtime - P1
3. Pas de staging parity validée - P1
4. Pas de rollback facile - P1
...
## Instructions
Commence par analyser le CI/CD actuel et identifier les opportunités d'amélioration. Implémente les feature flags et le pipeline complet progressivement.
```
...
1. **Copier le prompt** de l'agent souhaité
2. **Coller dans votre outil** (Claude, ChatGPT, Cursor, etc.)
3. **L'agent analysera** le codebase et commencera les tâches
4. **Suivre les instructions** de l'agent pour validation

### Ordre Recommandé d'Exécution

**Semaine 1 (Critique)**:
1. Agent Sécurité (Tâches 1-2: Auth, CSP)
2. Agent Architecture (Tâche 1: TypeScript strict)
3. Agent Observabilité (Tâche 3: Logger erreurs API)

**Semaine 2 (Critique)**:
4. Agent Sécurité (Tâches 3-4: Rate limiting, Timeout)
5. Agent Tests (Tâche 2: E2E paiement)

**Semaine 3+ (Amélioration)**:
6. Agent Performance (Tâches 1-5)
7. Agent Data Layer (Tâche 1: React Query)
8. Agent UX (Tâches 1-5)
9. Agent DevOps (Tâches 1-5)

### Coordination

- Les agents peuvent travailler en parallèle sur des fichiers différents
- Communiquer les changements majeurs (ex: TypeScript strict affecte tous les agents)
