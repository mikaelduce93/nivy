-- Migration 076: recommend_for_teen v2 — cohort-aware quiz filter
-- TICKET-007 (Wave 2, Sub-agent Q2)
--
-- Changes vs v1 (migration 052):
--   - Quiz candidate set is filtered by cohort_key match. A quiz is
--     eligible for a teen iff:
--       cohort_key IS NULL                              -- universal/library
--       OR cohort_key = derive_quiz_cohort_key(teen)    -- exact match
--     This stops cohort-specific AI quizzes (e.g. 4eme/public/maroc/fr)
--     from leaking to teens in unrelated cohorts.
--   - The 7-day no-repeat penalty (`quiz_seen_history`) is preserved.
--   - Affinity / novelty / context / difficulty scoring unchanged.
--   - Mission / event / partner_offer branches unchanged.
--
-- Acceptance: a teen at (4eme, public, maroc, fr) only receives AI
-- quizzes whose cohort_key matches OR is NULL.
...
BEGIN
  ...
  -- Derive the teen's cohort_key once. Mirrors derive_quiz_cohort_key()
  -- and the TS `cohortId()` helper. teens.school_type / curriculum may
  -- be NULL on legacy rows → falls through to 'unknown' so we still
  -- only match quizzes whose cohort_key is explicitly NULL OR identical.
  ...
  candidates AS (
    SELECT id, tags, title, difficulty_num, est_minutes FROM (
      -- Quiz branch — cohort-scoped (v2 change)
      SELECT id,
       ...
       WHERE p_content_type = 'quiz' AND is_active = TRUE
         -- Cohort filter: NULL cohort_key = universal/library; otherwise must match teen
