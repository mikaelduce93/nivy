    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
      {DROPS.map((drop, idx) => (
        <motion.div
          ...
          viewport={{ once: true }}
          className="group relative overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[3rem] bg-card border border-ink shadow-2xl"
        >
          {/* Image Background */}
          <div className="absolute inset-0 z-0">
            <Image
              ...
              unoptimized
            />
            <div className="absolute inset-0 bg-gradient-to-t from-paper-2 via-paper-2 to-transparent" />
          </div>
          ...
          <div className="relative z-10 p-5 sm:p-7 md:p-10 h-full flex flex-col justify-between space-y-6 sm:space-y-8 md:space-y-12">
            <div className="flex justify-between items-start gap-2">
              <div className="space-y-1.5 sm:space-y-2 min-w-0">
                <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-paper-2 border border-ink ">
                  <Timer className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-accent-soft shrink-0" />
                  <span className="text-[7px] sm:text-[8px] font-black text-ink uppercase tracking-widest truncate">{drop.timeLeft} LEFT</span>
                </div>
                <h3 className="text-xs sm:text-sm font-black text-mute uppercase tracking-[0.2em] sm:tracking-[0.3em] truncate">{drop.brand}</h3>
              </div>
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              <h4 className="text-xl sm:text-2xl md:text-3xl font-black text-ink tracking-tighter uppercase italic leading-none">
                {drop.offer}
              </h4>
              
              <Button className="w-full h-12 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl bg-white text-ink font-black text-sm sm:text-base md:text-lg hover:scale-[1.02] transition-all group/btn">
