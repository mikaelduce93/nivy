...
export function CrewHub() {
  ...
  return (
    ...
    >
      <div className="relative h-full rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden bg-background border border-ink">
        {/* Animated header gradient */}
        <div className="relative h-20 sm:h-24 overflow-hidden">
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-pink via-pink to-pink"
        ...
        <div className="p-4 sm:p-6 md:p-8 -mt-12 sm:-mt-14 relative z-10 space-y-4 sm:space-y-6">
          ...
          <div className="flex items-end justify-between gap-4">
            <div className="flex items-end gap-3 sm:gap-4">
              ...
              >
                ...
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-[1.75rem] sm:rounded-[2rem] bg-card border-4 border-ink flex items-center justify-center shadow-2xl overflow-hidden">
                  {crew?.avatar_url ? (
                    ...
                    <Users className="w-8 h-8 sm:w-10 sm:h-10 text-pink" />
                  )}
                </div>
              ...
              <div className="pb-1 sm:pb-2 min-w-0">
                <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-ink tracking-tighter uppercase truncate">
                  {crew?.name}
                </h3>
                <p className="text-mute font-bold text-[10px] sm:text-xs uppercase tracking-[0.15em] sm:tracking-[0.2em] truncate">
            ...
            <CursorHoverArea variant="pointer" magnetic={0.2}>
              ...
              >
                ...
                >
                  ...
                  >
                    <p className="text-[8px] sm:text-[10px] font-black uppercase text-mute">Rang</p>
                    <p 
            ...
            {[
              ...
              <CursorHoverArea key={stat.label} variant="pointer" magnetic={0.15}>
                <motion.div
                  ...
                  className="p-3 sm:p-4 rounded-2xl sm:rounded-2xl bg-paper-2 border border-ink space-y-1 hover:border-ink transition-colors"
                >
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    ...
                    <span className="text-[8px] sm:text-[10px] font-black uppercase text-mute">{stat.label}</span>
                  </div>
                  <p className="text-lg sm:text-xl font-black text-ink">
          ...
          <div className="space-y-2 sm:space-y-3">
            <div className="flex justify-between items-center text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-mute">
              <span>Progression Tier</span>
              <motion.span 
                className="text-pink flex items-center gap-1"
            ...
            <div className="relative h-2 sm:h-2.5 bg-paper-2 rounded-full overflow-hidden border border-ink">
              <motion.div
                className="h-full rounded-full bg-gradient-to-r from-pink via-pink to-pink"
                initial={{ width: 0 }}
                ...
                >
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="relative">
                      <div className="relative w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-card overflow-hidden border border-ink">
                        <Image
                    ...
                    <span className="text-xs sm:text-sm font-bold text-ink tracking-tight">{member.pseudo}</span>
                  </div>
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <span className="text-[9px] sm:text-[10px] font-black text-mute">Lvl {member.level}</span>
                    <motion.div 
                      className="px-1.5 sm:px-2 py-0.5 rounded-full bg-pink/10 text-[7px] sm:text-[8px] font-black text-pink uppercase"
            ...
            <Link href="/teen/circles?action=battle">
              ...
              >
                <PremiumButton
                  variant="default"
                  size="lg"
                  className="w-full h-12 sm:h-14 rounded-xl sm:rounded-2xl bg-white text-ink font-black text-sm sm:text-lg"
                  glow
                  glowColor="#8b5cf6"
                  ripple
                >
      ...
      >
        <div className="relative h-full rounded-[2rem] bg-gradient-to-br from-pink/40 via-paper-2 to-black overflow-hidden">
          ...
          <div className="relative z-10 p-6 sm:p-8 flex flex-col items-center text-center h-full justify-center space-y-4 sm:space-y-6">
            ...
            >
              <motion.div
                className="absolute -inset-4 rounded-[2rem] bg-pink/20"
                animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
                transition={{ duration: 3, repeat: Infinity }}
              />
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] sm:rounded-[2rem] bg-pink/20 flex items-center justify-center border border-pink/30">
                <Users className="w-8 h-8 sm:w-10 sm:h-10 text-pink" />
              </div>
            </motion.div>
            
            <div className="space-y-2">
              <h3 className="text-xl sm:text-2xl font-black text-ink tracking-tight">PAS DE CREW ?</h3>
              <p className="text-mute max-w-[240px] text-xs sm:text-sm">
...
function CrewHubSkeleton() {
  return (
    <div className="h-full rounded-[2.5rem] bg-card overflow-hidden animate-pulse">
      <div className="h-24 bg-gradient-to-r from-pink/20 to-pink/20" />
      <div className="p-8 -mt-14 space-y-6">
        <div className="flex items-end gap-4">
          <div className="w-24 h-24 rounded-[2rem] bg-paper-2" />
          <div className="space-y-2 pb-2">
            <div className="h-8 w-32 rounded-lg bg-paper-2" />
            <div className="h-4 w-24 rounded bg-paper-2" />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-20 rounded-2xl bg-paper-2" />
          ))}
        </div>
        <div className="h-3 rounded-full bg-paper-2" />
        <div className="space-y-2">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-14 rounded-2xl bg-paper-2" />
          ))}
        </div>
        <div className="h-14 rounded-2xl bg-paper-2" />
      </div>
    </div>
  )
}
