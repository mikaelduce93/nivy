...
// G2-C (audit 2026-07-11, friction 7) — planche de complétion des quêtes
// quotidiennes (user_challenges, moteur n°1). Extraite de l'ancienne page
// publique /daily (Navbar/Footer du site) pour être montée dans l'univers
// teen (/teen/daily, chrome 5 piliers). La résolution du teen se fait côté
// serveur (getMyTeenSelf) : ce composant reçoit un teen déjà résolu — plus
// de sélecteur multi-enfants (héritage du hub « parent-flavored » /daily,
// qui était de toute façon inatteignable : next.config redirigeait /daily).
...
interface DailyChallengesBoardProps {
  teenId: string
  pseudo?: string | null
}

export function DailyChallengesBoard({ teenId, pseudo }: DailyChallengesBoardProps) {
  ...
  // Load stats and challenges on mount
  useEffect(() => {
    if (!teenId) return
    ...
    async function loadData() {
      ...
      try {
        const [statsResult, challengesResult] = await Promise.all([
          getTeenGamificationStats(teenId),
  ...
  const handleSkipChallenge = async (challengeId: string) => {
    try {
      const result = await skipChallenge(challengeId, teenId)

      if (result.success) {
        toast("Défi passé", { description: "Tu peux le faire demain !" })

        // Reload challenges
        const challengesResult = await getDailyChallenges(teenId)
        if (challengesResult.success) setChallenges(challengesResult.data)
      } else {
        toast.error("Erreur")
      }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10 pb-8">
      ...
      <section className="pt-4 text-center">
        ...
        <h1 className="mt-2 font-display text-3xl font-extrabold leading-[1.04] tracking-tight sm:text-4xl lg:text-5xl">
          Tes défis <em className="font-semibold italic text-pink">du jour</em>
          {pseudo && <span className="block text-pink">@{pseudo}</span>}
        </h1>
            ...
            <NivCelebration
              ...
              action={
                <div className="flex flex-wrap justify-center gap-3">
                  {/* G2-C — CTAs internes à l'univers teen (l'ancien /daily
                      renvoyait vers /evenements et /profile, hors app). */}
                  <Button variant="outline" onClick={() => router.push('/teen/events')}>Voir les événements</Button>
                  <Button variant="pink" onClick={() => router.push('/teen/quests')}>Mes quêtes</Button>
                </div>
              }
            />
          </div>
